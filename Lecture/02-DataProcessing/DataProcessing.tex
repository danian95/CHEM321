...
\frame{\section{Significant Figure Review}
	\begin{learningobjectives}
		...
		\item Explain why certain digits are significant.
...
\frame{\section{Error}
	\begin{learningobjectives}
		\item Explain the types and causes of experimental error.
...
\begin{frame}{Propagation of Random Error}
		Systematic error can be removed by \emph{calibrating} glassware,
		but one can \emph{never} remove \alert{random error}.
...
\begin{frame}{More Propagation of Random Error}
	\begin{center}
		\resizebox{0.9\linewidth}{!}{
...
%\begin{frame}[t]{Propagation of Error Example}
%	For a \SI{1.0}{\milli\Molar} Au sol'n,
%		\begin{itemize}
%			\item An analytical balance was used to weigh
%				\SI{0.0396(5)}{\gram} \ch{HAuCl4 * 3 H2O} (FW =
%				\SI{393.82}{\gram\per\mole})\footnote{Error in
%					molar mass is often much smaller than
%				our laboratory error.}
%
...
%				\note<2>{%
%					\begin{align*}
%						\%e_{\text{g}/\text{L}} &= \sqrt{\%e^2_{\text{g~Au}} + \%e^2_{\text{volume~\ch{H2O}}}} \\
...
\frame{\section{Statistics}
	\begin{learningobjectives}
		\item Define the Gaussian distribution.
...

{
	...
	\begin{frame}
		\begin{exampleblock}{Harris, 9\textsuperscript{th} ed., pg. 64}
...
\frame{\section{Comparison of Means}
	\begin{learningobjectives}
		\item Compare results with a ``known'' or previously
			obtained value(s).
		\item Identify $F$ and $t$ in tables for a particular confidence interval.
		\item Explain if a result significantly differs from another.
		\item Identify and eliminate outliers.
	\end{learningobjectives}
}
...
\begin{frame}{Case 2: Replicate measurements
	(Unpaired)}
	\begin{itemize}
...
\begin{learningobjectives}
\item Explain the different types of calibration methods.
\item Identify when a certain curve may be perferable over other
	calibration methods.
