...
\begin{frame}{Manipulating Equilibrium Constants}
	...
	\begin{align*}
		...
		\intertext{Reversing the reaction \alert{inverses} the value of
		$K$:}
		\ch{H+ + A-} &\ch{<=>} \ch{HA} &&& K_1' &=
...
%	\begin{enumerate}
%			\small
%		\item Dissolve \ch{CaCO3} -- Solubility $\Ksp{}$
...
%		reaction.  The heat absorbed or released (\alert{enthalpy}) and
%		the dispersal of energy into molecular motion (\alert{entropy})
	...
	\begin{itemize}
		...
		\item The \alert{reaction quotient} predicts the direction of
			...
			\begin{align*}
				...
				{[\ch{A}]^a[\ch{B}]^b}
			\end{align*}
			when A, B, C, and/or D are \alert{not} at equilibrium
			concentrations.
			\begin{center}
				\parbox{0.8\linewidth}{
					\begin{itemize}
...
\begin{frame}{Formation: Lewis Acid/Base Reactions}
	...
	\visible<2->{\textbf{What product is present?}}
}
	...
	\begin{itemize}
		\item<4-> $K_{\mathrm{i}}$ will be used for \alert{stepwise} formation
			constants:
			\begin{center}
			...
			\end{tabular}
			\end{center}
		\item<5-> $\beta_{\mathrm{i}}$ will be used for \alert{cumulative} formation
			constants:
			\begin{center}
...
\begin{frame}{Why is the equilibrium constant not enough?}
	\only<1-3>{%
...
\begin{frame}{Charge Balance}
	\begin{itemize}
		...
		\item Activity coefficients \alert{do not} appear in the charge
			balance!
	\end{itemize}
\end{frame}
...
\begin{frame}{The Systematic Treatment of Equilibrium}
	\begin{enumerate}
		\item Write the \alert{pertinent reactions}.
		\item Write the \alert{charge balance} equation.
		\item Write the \alert{mass balance} equation(s).
		\item Write the \alert{equilibrium constant} for each chemical
			reaction.
			\begin{itemize}
				\item This is the only step where activity
					coefficients may appear.
			\end{itemize}
		\item \alert{Count} the equations and unknowns.
			\begin{itemize}
				\item The numbers should equal or you need to
					find more equilibria or fix some
					concentrations.
			\end{itemize}
		\item \alert{Solve} for all the unknowns.
	\end{enumerate}
\end{frame}
